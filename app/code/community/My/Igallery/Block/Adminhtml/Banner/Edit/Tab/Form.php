...
class My_Igallery_Block_Adminhtml_Banner_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $layoutFieldset->addField('is_parent', 'select', array(
            'label'     => Mage::helper('igallery')->__('Is Parent Gallery'),
            'name'      => 'is_parent',
			'values'    => Mage::getSingleton('adminhtml/system_config_source_yesno')->toOptionArray(),
		...
		foreach($blockData as $k => $v){
			$blockOptions[] = array('value'=>$v['identifier'], 'label'=>$v['title']);
		}
		array_unshift($blockOptions, array('value'=>'', 'label'=>'Please select a static block ...'));

		$layoutFieldset->addField('cms_block', 'select', array(
			'name'     => 'cms_block',
			'label'    => Mage::helper('igallery')->__('INFO Tab Content'),
			'values'   => $blockOptions,
