...
class AW_Blog_Manage_BlogController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            if(isset($_FILES['feature_media']['name']) && $_FILES['feature_media']['name'] != '') {

                $id = $this->getRequest()->getParam('id');
                $model->load($id);

            try{
                $uploader = new Varien_File_Uploader('feature_media');
                ...
                $media_path  = Mage::getBaseDir('media') . DS . 'feature_media' . DS;

                // Set thumbnail name
                $file_name = 'cms_';

                // Upload the image
                $uploader->save($media_path, $file_name . $_FILES['feature_media']['name']);

                $data['feature_media'] = 'feature_media' . '/' . $file_name . $_FILES['feature_media']['name'];

                // Set thumbnail name
                $data['feature_media'] = $data['feature_media'];
                $model->setFeatureMedia($data['feature_media']);
            }
            catch (Exception $e)
            {
                ...
                if($data['feature_media']['delete'] == 1) {
                    $data['feature_media'] = '';
                    $model->setFeatureMedia($data['feature_media']);
                } else {
                    unset($data['feature_media']);
                    $model->setFeatureMedia(implode($data['feature_media']));
                }
            }
