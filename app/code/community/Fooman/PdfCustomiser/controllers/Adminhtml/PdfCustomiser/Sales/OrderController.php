...
class Fooman_PdfCustomiser_Adminhtml_PdfCustomiser_Sales_OrderController extends Fooman_EmailAttachments_Admin_OrderController
{
     ...
     * @return Mage_Core_Controller_Varien_Action
     */
    public function pdfinvoicesshipmentsAction()
    {
        $orderIds = $this->getRequest()->getPost('order_ids');
        if (sizeof($orderIds)) {
            $pdf = Mage::getModel('pdfcustomiser/invoice')->renderPdf(null, $orderIds, null, true);
            Mage::getModel('pdfcustomiser/shipment')->renderPdf(null, $orderIds, $pdf);
        } else {
            $this->_getSession()->addError($this->__('There are no printable documents related to selected orders'));
        }
        $this->_redirectReferer('adminhtml/sales_order');
    ...
    public function printAction()
    {
        ...
        if ($orderId) {
            Mage::getModel('pdfcustomiser/order')->renderPdf(null, array($orderId));
        } else {
    ...
    public function pdfdocsAction()
    {
        ...
        if (sizeof($orderIds)) {
            if (!Fooman_PdfCustomiser_Model_Abstract::COMPAT_MODE) {
                $pdf = Mage::getModel('pdfcustomiser/invoice')->renderPdf(null, $orderIds, null, true);
                $pdf = Mage::getModel('pdfcustomiser/shipment')->renderPdf(null, $orderIds, $pdf, true);
    ...
    public function pdfpickinglistAction()
    {
        $orderIds = $this->getRequest()->getPost('order_ids');
        if (sizeof($orderIds)) {
            Mage::getModel('pdfcustomiser/order_pickingList')->renderPdf(null, $orderIds);
        } else {
            $this->_getSession()->addError($this->__('There are no printable documents related to selected orders'));
        }
        $this->_redirectReferer('adminhtml/sales_order');
