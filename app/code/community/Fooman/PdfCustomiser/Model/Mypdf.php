...
class Fooman_PdfCustomiser_Model_Mypdf extends TCPDF
{
    ...
    {
        $return = array();
        $return['Name'] = $this->_fixEncoding($item->getName());
        $return['Sku'] = $this->_fixEncoding($item->getSku());
        ...
        if ($options) {
                ...
                foreach ($options['options'] as $option) {
                    if ($option['label'] == 'Detail') {
                        foreach (explode("\n", $option['value']) as $detailLines) {
                            $return['Options'] .= "<br/>&nbsp;&nbsp;" . $this->_fixEncoding($detailLines);
                        }
                    } else {
                        //Aitoc Custom Product Options adds a whole bunch of JS
                        //filter it out here
                        if (strpos($option['value'], 'Aitcg.Main') === false) {
                            $printValue = isset($option['print_value']) ? $option['print_value'] : $option['value'];
                            $return['Options'] .= "<br/>&nbsp;&nbsp;" . $this->_fixEncoding($option['label'] . ": " . $printValue);
                        }
                    }
                }
                $return['Options'] .= "<br/>";
            ...
            if (isset($options['additional_options'])) {
                foreach ($options['additional_options'] as $additionalOption) {
                    $return['Options'] .= "<br/>&nbsp;&nbsp;" . $this->_fixEncoding(
                    ...
                    if ($attributeCode) {
                        ...
                        if ($attribute) {
                            if ($attribute->getBackendModel() == 'weee/attribute_backend_weee_tax') {
                                ...
                                $attributeValue = $this->_fixEncoding($product->getAttributeText($attributeCode));
                                if (is_array($attributeValue) && !empty($attributeValue)) {
                                    ...
                                    if (!$return[$customName]) {
                                        $return[$customName] = $this->_fixEncoding($product->getDataUsingMethod($attributeCode));
                                    }
                                }
                            }
                        }
    ...
    {
        $payment = $order->getPayment()->getMethodInstance()->setStore($order->getStoreId());
        $theme = (string)Mage::getStoreConfig('design/admin/theme');
        if (empty($theme)) {
            $theme = 'default';
        }
    ...
    public function PrepareShipping(Fooman_PdfCustomiser_Helper_Pdf $helper, $order, $printItem)
    {
        if (!$order->getIsVirtual()) {
            ...
            $html = $block->setPdfHelper($helper)
                ...
                ->setShippingDescription($this->_fixEncoding($order->getShippingDescription()))
    ...
    {
        return htmlspecialchars_decode($this->_fixEncoding($string));
    }

    protected function _fixEncoding($input)
    {
        if(is_array($input)){
            $returnArray = array();
            foreach ($input as $string){
                $returnArray[] = $this->_fixEncoding($string);
            }
            return $returnArray;
        }
