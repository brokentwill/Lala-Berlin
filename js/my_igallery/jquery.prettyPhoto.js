...
(function ($) {
    ...
    $.fn.prettyPhoto = function (pp_settings) {
      ...
      <a class="show-all"><span class="txt">SHOW ALL</span><span class="icon"></span></a> \
      <span class="s-plus"></span> \
      <span class="s-minus"></span> \
      <div class="images-zoom-wrap"><img src=""></div> \
     ...
     <div class="pp_overlay"></div>',
        ...
        <div> \
         <ul> \
          {gallery} \
            ...
            custom_markup: '',
            social_tools: "<a class='a_zoom link' onclick='window.open(\"{data-url}\", \"_blank\", \"width=450, height=350, top=220, left=320\"); return false;'><span class='txt'>Share Look</span><span class='icon'></span></a><br /><a class='a_shop link' href='{data-shop}'><span class='txt'>Shop Look</span><span class='icon'></span></a>"
        }, pp_settings);
        ...
        $.prettyPhoto.initialize = function () {
            ...
            pp_dataUrl = (isSet) ? jQuery.map(matchedObjects, function (n, i) {
                if ($(n).attr(settings.hook).indexOf(theRel) != -1) return ($(n).attr('data-url')) ? $(n).attr('data-url') : "";
            }) : $.makeArray($(this).attr('data-url'));

            pp_dataShop = (isSet) ? jQuery.map(matchedObjects, function (n, i) {
                if ($(n).attr(settings.hook).indexOf(theRel) != -1) return ($(n).attr('data-shop')) ? $(n).attr('data-shop') : "";
            }) : $.makeArray($(this).attr('data-shop'));

            pp_small = (isSet) ? jQuery.map(matchedObjects, function (n, i) {
                if ($(n).attr(settings.hook).indexOf(theRel) != -1) return $(n).attr('data-big');
            }) : $.makeArray($(this).attr('data-big'));
        ...
        $.prettyPhoto.open = function (event) {
            if (typeof settings == "undefined") {
                ...
                pp_dataUrl = (arguments[4]) ? $.makeArray(arguments[4]) : $.makeArray("");
                pp_dataShop = (arguments[5]) ? $.makeArray(arguments[5]) : $.makeArray("");
                pp_small = (arguments[6]) ? $.makeArray(arguments[6]) : $.makeArray("");
            ...
            if (settings.social_tools) {
                facebook_like_link = settings.social_tools.replace('{data-url}', pp_dataUrl[set_position]);
                facebook_like_link = facebook_like_link.replace('{data-shop}', pp_dataShop[set_position]);
                $pp_pic_holder.find('.pp_social').html(facebook_like_link);

                $pp_pic_holder.find('.pp_details .pp_social .a_shop').hover(function() {
                    $(this).find('.txt').animate({'margin-right': '-33px'}, 500);
                }, function() {
                    $(this).find('.txt').animate({'margin-right': '-125px'}, 500);
                });

                $pp_pic_holder.find('.pp_details .pp_social .a_zoom').hover(function() {
                    $(this).find('.txt').animate({'margin-right': '-33px'}, 500);
                }, function() {
                    $(this).find('.txt').animate({'margin-right': '-125px'}, 500);
                });
            }
            if (typeof pp_dataShop[set_position] != 'undefined' && pp_dataShop[set_position] != "") {
                $pp_pic_holder.find('.pp_details .pp_social .a_shop').fadeIn();
            } else {
                $pp_pic_holder.find('.pp_details .pp_social .a_shop').fadeOut();
            }
            ...
            if (typeof pp_descriptions[set_position] != 'undefined' && pp_descriptions[set_position] != "") {
                ...
                $pp_pic_holder.find('.pp_description').fadeOut();
            }
        ...
        function _showContent() {
            ...
            $pp_pic_holder.find('.pp_content').animate({
                ...
                $pp_gallery.animate({'opacity': 1}, settings.animation_speed);
            });
        ...
        function _build_overlay(caller) {
            ...
            if (isSet && settings.overlay_gallery) {
                currentGalleryPage = 0;
                toInject = "";
                for (var i = 0; i < pp_small.length; i++) {
                    if (!pp_small[i].match(/\b(jpg|jpeg|png|gif)\b/gi)) {
                        classname = 'default';
                        img_src = '';
                    } else {
                        classname = '';
            ...
            //XX
            $pp_pic_holder.find('.pp_content').click(function() {

                // var baseUrl = window.location.protocol + "//" + window.location.host + "/skin/frontend/default/default/my_igallery/images/";
                

                var img = $(this).find('#pp_full_res img');
                var url = img.attr('src');
                var item = $('.images-zoom-wrap img');
            ...
            $pp_pic_holder.find('.pp_content_container .show-all').hover(function() {
                $(this).find('.txt').animate({'margin-right': '-33px'}, 500);
            }, function() {
                $(this).find('.txt').animate({'margin-right': '-125px'}, 500);
            });
            ...
            // $pp_pic_holder.find('.pp_content .pp_fade #pp_full_res').on('mouseenter', function(e){
    
            // });

            // $pp_pic_holder.find('.pp_content .pp_fade').on('mousemove', function(e){
